...
class ALBNoSQLDBObject {
    ...
    func jsonValue() -> String {
        let dataValue = NSJSONSerialization.dataWithJSONObject(dictionaryValue(), options: NSJSONWritingOptions(0), error: nil)
...
final class ALBNoSQLDB {
    ...
    */
    ...
    :param: sortOrder Optional string that gives a comma delimited list of properties to sort by
    ...
    */
    ...
    :param: table The table to return keys from.
    :param: sortOrder Optional string that gives a comma delimited list of properties to sort by
    :param: conditions Array of DBConditions that specify what conditions must be met.
    ...
    */
    class func keysInTableForConditions(table:String, sortOrder:String?, conditions:[DBCondition]) -> [String]? {
        ...
        if let results = db.sqlSelect("select arrayColumns from __tableArrayColumns where tableName = '\(table)'") {
            if results.count > 0 {
                arrayColumns = split(results[0].values[0] as! String) { $0 == "," }
            }
        } else {
            return nil
        }
        ...
        if conditionSet.count > 0 {
            ...
            for condition in conditionSet {
                if tableColumns.filter({$0 == condition.objectKey}).count == 0 && arrayColumns.filter({$0 == condition.objectKey}).count == 0 {
                    println("\(condition.objectKey) doesn't exist")
                    return []
                }
                ...
                default:
                    if let text = condition.value as? String {
...
//        println(sql)
    ...
    :param: table The table to return keys from.
    :param: indexes An array of table properties to be indexed. An array can be compound.
    */
    ...
    :param: value A JSON string representing the value to be stored. Top level object provided must be a dictionary.
    :param: autoDeleteAfter Optional date of when the value should be automatically deleted from the table.
    
    :returns: Bool If the value was set successfully.
    */
    class func setValue(#table:String, key:String, value:String, autoDeleteAfter:NSDate? = nil) -> Bool {
        let db = ALBNoSQLDB.sharedInstance
        assert(table != "", "table name must be provided")
        assert(!db.reservedTable(table),"reserved table")
    ...
    */
    ...
    :param: tables Array of tables that are not to be synced.
    ...
    */
    ...
    :param: filePath The full path, including the file itself, to be used for the log file.
    :param: lastSequence The last sequence used for the given target DB. Initial sequence is 0.
    :param: targetDBInstanceKey The dbInstanceKey of the target database. Use the class dbInstanceKey method to get the DB's instanceKey.
        ...
        if let fileHandle = NSFileHandle(forWritingAtPath: filePath) {
            if let results = db.sqlSelect("select rowid,timestamp,originalDB,tableName,activity,key from __synclog where rowid > \(lastSequence) and sourceDB <> '\(targetDBInstanceKey)' and originalDB <> '\(targetDBInstanceKey)' order by rowid") {
                ...
                for row in results {
                    ...
                    let dataValue = NSJSONSerialization.dataWithJSONObject(entryDict, options: NSJSONWritingOptions(0), error: nil)
                    if firstEntry {
                        firstEntry = false
                    } else {
    ...
    */
    ...
    class func processSyncFileAtURL(localURL:NSURL!, syncProgress:syncProgressUpdate?)->(Bool,String,Int) {
        ...
        if let fileHandle = NSFileHandle(forReadingAtPath: filePath) {
            ...
            let now = ALBNoSQLDB.stringValueForDate(NSDate())
            if let fileText = String(contentsOfFile: filePath, encoding: NSUTF8StringEncoding, error: nil) {
                let dataValue = fileText.dataValue()
                
                if let objectValues = NSJSONSerialization.JSONObjectWithData(dataValue, options: NSJSONReadingOptions.MutableContainers, error: nil) as? [String:AnyObject] {
    ...
    */
    class func guid() -> String {
        ...
        var uuidString = String(CFUUIDCreateString(kCFAllocatorDefault, uuid))
    ...
    :param: date Date to get string value of
    ...
    */
    ...
    :param: stringValue String representation of date given in ISO format "yyyy-MM-dd'T'HH:mm:ss'.'SSSZZZZZ"
    ...
    */
    ...
    class var sharedInstance : ALBNoSQLDB {
        struct Static {
            static let instance = ALBNoSQLDB()
        }
    ...
    private func openDBFile(dbFilePath:String) -> Bool {
        ...
        if status != SQLITE_OK {
            println("Error opening SQLite Database: \(status)")
            return false
        }
    ...
    private func checkSchema() {
        ...
        if let schemaResults = sqlSelect("select value from __settings where key = 'schema'") {
            var schemaVersion = (schemaResults[0].values[0] as! String).toInt()!
            if schemaVersion == 1 {
    ...
    private func setValue(#table:String, key:String, objectValues:[String:AnyObject], addedDateTime:String, updatedDateTime:String, deleteDateTime:String, sourceDB:String, originalDB:String) -> Bool {
        if !openDB() {
            return false
        }
        ...
        let joinedArrayKeys = ",".join(arrayKeyTypes)
        ...
        if let results = sqlSelect(sql) {
            if results.count == 0 {
                ...
                for column in columns {
                    let filteredKeys = objectKeys.filter({$0==column.name}).array
    ...
    private func processValues(#table:String, objectValues:[String:AnyObject], sql:String) -> Bool {
    ...
    private func bindValue(statement:COpaquePointer, index:Int32, value:AnyObject) -> Int32 {
        ...
        var valueType = typeOfValue(value)
    ...
    private func deleteForKey(#table:String, key:String, autoDelete:Bool, sourceDB:String, originalDB:String) -> Bool {
        if !openDB() {
            return false
        }
    ...
    private func dictValueForKey(#table:String, key:String, includeDates:Bool) -> [String:AnyObject]? {
        ...
        let arrayObjects = split(results![0].values[0] as! String) { $0 == "," }
        for object in arrayObjects {
            if object == "" {
                continue
            }
            
            let keyType = split(object) { $0 == ":" }
    ...
    private func validateTableColumns(#table:String, objectValues:[NSString:AnyObject]) {
        let columns = columnsInTable(table)
        ...
        for (objectKey,value) in objectValues {
            ...
            if !found {
                ...
                if valueType == .int || valueType == .double || valueType == .string {
                    ...
                    // array type
                    var sql = "select arrayColumns from __tableArrayColumns where tableName = '\(table)'"
                    if let results = sqlSelect(sql) {
                        var arrayColumns = ""
                        if results.count > 0 {
...
//        explain(sql)
    ...
    private func explain(sql:String) {
        ...
        let status = sqlite3_prepare_v2(self._sqliteDB, explainCommand, -1, &dbps, nil)
        println("\n\n.  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  \nQuery:\(sql)\n\nAnalysis:\n")
        while (sqlite3_step(dbps) == SQLITE_ROW) {
            ...
            println("\(iSelectid) \(iOrder) \(iFrom) \(value)\n=================================================\n\n")
        }
